...
get_locations <- function(assay, plate){
  ...
  triplicate_384 <- data.frame(position_name = c('pos_1','pos_2','pos_3','pos_4','pos_5',
                               ...
                               set_rows_t = c(11,14,17,20,23,11,14,17,20,23),
                               set_rows_b = c(13,16,19,22,25,13,16,19,22,25),
                               set_cols_l = c(3,3,3,3,3,15,15,15,15,15),
                               set_cols_r = c(12,12,12,12,12,24,24,24,24,24))
  
  duplicate_384 <- data.frame(position_name = c('pos_1','pos_2','pos_3','pos_4','pos_5',
                                                'pos_6','pos_7','pos_8','pos_9','pos_10',
                                                'pos_11','pos_12','pos_13','pos_14'),
                              set_rows_t = c(11,13,15,17,19,21,23,11,13,15,17,19,21,23),
                              set_rows_b = c(12,14,16,18,20,22,24,12,14,16,18,20,22,24),
                              set_cols_l = c(3,3,3,3,3,3,3,15,15,15,15,15,15,15),
                              set_cols_r = c(12,12,12,12,12,12,12,24,24,24,24,24,24,24))
  ...
  duplicate_96 <- data.frame(position_name = c('pos_1','pos_2','pos_3'),
                             set_rows_t = c(11,13,15),
                             set_rows_b = c(12,14,16),
                             set_cols_l = c(2,2,2),
                             set_cols_r = c(11,11,11))
  ...
  if (assay$format == '384w'){
    ...
    locate$blank <-  mean(as.numeric(as.character(plate[11:24,14])))
    locate$no_drug <- mean(as.numeric(as.character(plate[11:24,13])))
  ...
  if (assay$format == '96w'){
    ...
    locate$blank <-  mean(as.numeric(as.character(plate[11:16,13])))
    locate$no_drug <- mean(as.numeric(as.character(plate[11:16,12])))
  }
